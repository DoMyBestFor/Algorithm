...
public class B16236 {
	/**
	 * 백준 아기 상어
	 * N X N 크기의 맵. M마리의 물고기와 아기 상어 1마리.
	 * 아기 상어 크기 : 2, 사방 이동
	 * 자신의 크기보다 큰 물고기가 있는 칸은 갈 수 없음.
	 * 자기보다 작은 물고기는 먹을 수 있음. 같은 물고기는 지날 수는 있지만 먹지는 못함.
	 * 먹을 수 있는 물고기가 1마리라면 그 물고기 먹으러 가고 1마리 초과면 가까운 물고기 먹으러 간다.
	 * 거리 : 지나는 칸의 개수
	 * 거리마저 같다면 가장 위에 있는 물고기 그것 마저 같으면 가장 왼쪽에 있는 물고기를 먹는다.
	 * 
	 * 자신의 크기와 같은 수의 물고기를 먹을 때마다 크기 1 증가한다. 크기 2는 2마리 먹어야 증가한다.
	 * 먹을 수 있는 물고기 없으면 엄마에게 도움 요청, 시뮬 종료. 몇초 동안 도움 요청 안하고 먹을 수 있나?
	 * = 몇초가 지나면 먹을 물고기가 없어지나?
	 * @param args
	 * @throws Exception
	 */
	static int N;
		...
		for(int i = 0; i < N; i++) {
			...
			for(int j = 0; j < N; j++) {
				...
				if(map[i][j] == 9) {
					x = i; // 아기 상어의 행 좌표
					y = j; // 아기 상어의 열 좌표
					map[i][j] = 0; // 아기 상어 좌표는 0으로 만들어야 함. 그래야 다음에 다시 올 수 있음
				}
			}
		}
